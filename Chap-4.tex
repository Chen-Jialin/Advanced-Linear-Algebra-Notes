...
$\forall r\in R$, 可构造映射 $\bar{r}:M\rightarrow M$, $m\mapsto rm$. $\bar{r}$ 是 $M$ 上的群同态, 又称\textbf{自同态}, 记作 $\bar{r}\in\End(M)\equiv\{\text{$M$ 上的自同态}\}$, $\End(M)$ 关于同态的加法、复合成环, 其单位元为 $M$ 上的恒等映射, 记作 $1_M$, 故还可构造映射 $\phi:R\rightarrow\End(M)$, $r\mapsto\bar{r}$.
\begin{pf}
...
\begin{pf}
    ...
    \begin{itemize}
        \item[(1)] $\emptyset\neq S\subseteq R$
        \item[(2)] $\forall u,v\in S$, $\forall r,t\in R$, $ru+tv\in S$. 特别地, 令 $r=1$, $t=-1$, 得 $u-v\in S$, 令 $t=0$, 得 $ru\in S$
    \end{itemize}
    故 $S$ 为 $R$ 的理想.
\end{pf}
...
\begin{df}[商模]
    $S$ 是 $M$ 的子模, \textbf{商群} $\frac{M}{S}\equiv\{[v]\mid v\in M\}$.
\end{df}
