...
% 第 0 章: 代数学基础 Chap 0: Algebra Basics
...
\begin{df}[映射的复合]
    映射 $f:S_1\rightarrow S_2$, $g:S_2\rightarrow S_3$, 则称映射 $g\circ f:S_1\rightarrow S_3$, $a\mapsto g\circ f(a)\equiv g(f(a))$ 为 $f$ 和 $g$ 的\textbf{复合}.
\end{df}
...
\begin{itemize}
    ...
    \begin{pf}
        $a\cdot 0=0\cdot a=(0+0)\cdot a=0\cdot a+0\cdot a=0\cdot a+a\cdot 0\Longrightarrow 0\cdot a=a\cdot 0=0$.
    \end{pf}
...
\begin{df}[域的特征]
    $\chr F\equiv$ 使得 $n\cdot 1=\overbrace{1+1+\cdots+1}^{\text{$n$ 个 $1$ 相加}}=0$ 的最小正整数 $n$.
\end{df}
